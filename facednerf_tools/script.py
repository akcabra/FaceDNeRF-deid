...
for image_id in image_ids:
    for i in range(len(input_dict)):
        ...
        command = "CUDA_VISIBLE_DEVICES=" + str(gpu_id1)+" python -m facednerf_tools.gen_videos_from_given_latent_code --outdir='"+output_dir_video+"' --trunc=0.7 --npy_path '"+output_dir_video+"/checkpoints/"+image_id+".npy'   --network='"+output_dir_video+"/checkpoints/fintuned_generator.pkl' --sample_mult=2"
        print(command)
        os.system(command)
